/**
 * Run migration-v102.sql against Supabase database
 * Tabel task_deliverables — lampiran file/video via Google Drive di task
 *
 * Mengikuti pola run-migration-v101.mjs (RPC exec_sql → /pg/query fallback).
 * Bila kedua path gagal, jalankan manual isi supabase/migration-v102.sql
...
const SQL = readFileSync(new URL('../supabase/migration-v102.sql', import.meta.url), 'utf8');
...
async function main() {
  ...
  console.log('Menjalankan migration-v102 (tabel task_deliverables)…');
  ...
  if (r.ok) {
    ...
    console.error('\n→ Jalankan manual isi supabase/migration-v102.sql di Supabase SQL Editor.');
    process.exit(1);
  }
}
