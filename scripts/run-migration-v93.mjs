/**
 * Run migration-v93.sql against Supabase database
 * Fix: RLS chat_channel_members (creator boleh kelola member) + backfill owner grup orphan
 */
...
async function runMigration() {
  const rawSql = readFileSync('supabase/migration-v93.sql', 'utf8');

  console.log('📋 Running migration-v93.sql (fix RLS members + backfill owner)...');
  ...
  // Split by semicolons (handle dollar-quotes just in case)
  const statements = [];
  let current = '';
  ...
  if (failed > 0) {
    ...
    console.log('   File: supabase/migration-v93.sql');
    process.exit(1);
  }
}
