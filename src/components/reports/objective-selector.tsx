...
export function ObjectiveSelector({ value, onChange, className }: ObjectiveSelectorProps) {
  ...
  return (
    <div className={cn("relative", className)} data-objective-dropdown>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-[400px] max-w-[90vw] rounded-lg border border-border bg-surface shadow-xl">
          {/* Search */}
          <div className="border-b border-border p-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-border bg-white py-1.5 pl-7 pr-2 text-xs outline-none focus:border-primary"
                autoFocus
              />
            </div>
          </div>
          ...
          <div className="max-h-[350px] overflow-y-auto p-2">
            {filteredGroups.map((group) => (
              <div key={group.label} className="mb-2">
                <p className="mb-1 px-1 text-[9px] font-bold uppercase tracking-wider text-muted">
                  {group.label}
                </p>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected ? "bg-primary/10 ring-1 ring-primary/30" : "hover:bg-accent/10"
                        )}
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-medium text-gray-900">{obj.label}</span>
                          <span className="badge bg-gray-100 text-muted text-[8px]">
                            {obj.platform}
                          </span>
                        </div>
                        <p className="mt-0.5 text-[10px] text-muted">{obj.description}</p>
                      </button>
                    );
                  })}
                </div>
